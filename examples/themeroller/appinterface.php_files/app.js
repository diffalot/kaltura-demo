...
function updateCSS(locStr){
	//once 1.6 final is ready: jQuery("head").append('<link href="/themeroller/css/parseTheme.css.php?'+ locStr +'" type="text/css" rel="Stylesheet" />');
	jQuery("link[href*=parseTheme.css.php]:last").after('<link href="/themeroller/css/parseTheme.css.php?'+ locStr +'" type="text/css" rel="Stylesheet" />');
...
function formChange(){
	var locStr = jQuery('#themeRoller form').serialize();
...
function rollYourOwnBehaviors() {	
	...
	jQuery('li.state-default, div.state-default').hover(
	...
	jQuery('input.hex')
		.validHex()
		.keyup(function() {
			jQuery(this).validHex();
		})
		.click(function(){
			jQuery(this).addClass('focus');
			jQuery('#picker').remove();
			jQuery('div.picker-on').removeClass('picker-on');
			jQuery('div.texturePicker ul:visible').hide(0).parent().css('position', 'static');
			jQuery(this).after('<div id="picker"></div>').parent().addClass('picker-on');
	...
	jQuery('input, select')
	.focus(function() {
		jQuery('input.focus, select.focus').removeClass('focus');
		jQuery(this).addClass('focus');
	})
	.blur(function() {
	...
	jQuery('select.texture').each(function() {
		
		jQuery(this).after('<div class="texturePicker"><a href="#"></a><ul></ul></div>');
		var texturePicker = jQuery(this).next();
		...
		jQuery(this).find('option').each(function(){
			ul.append('<li class="'+ jQuery(this).attr('value') +'" data-texturewidth="'+jQuery(this).attr('data-texturewidth')+'" data-textureheight="'+jQuery(this).attr('data-texturewidth')+'" style="background: #555555 url(/themeroller/images/?new=555555&w='+jQuery(this).attr('data-texturewidth')+'&h='+jQuery(this).attr('data-textureheight')+'&f=png&q=100&fltr[]=over|textures/'+jQuery(this).attr('value')+'|0|0|100) 50% 50% repeat"><a href="#" title="'+ jQuery(this).text() +'">'+ jQuery(this).text() +'</a></li>');
			if(jQuery(this).get(0).index == sIndex){texturePicker.attr('title',jQuery(this).text()).css('background', '#555555 url(/themeroller/images/?new=555555&w='+jQuery(this).attr('data-texturewidth')+'&h='+jQuery(this).attr('data-textureheight')+'&f=png&q=60&fltr[]=over|textures/'+jQuery(this).attr('value')+'|0|0|100) 50% 50% repeat');}
		});
		
		ul.find('li').click(function() {
			texturePicker.prev().get(0).selectedIndex = texturePicker.prev().find('option[value='+ jQuery(this).attr('class') +']').get(0).index;
			texturePicker.attr('title',jQuery(this).text()).css('background', '#555555 url(/themeroller/images/?new=555555&w='+jQuery(this).attr('data-texturewidth')+'&h='+jQuery(this).attr('data-textureheight')+'&f=png&q=60&fltr[]=over|textures/'+jQuery(this).attr('class')+'|0|0|100)  50% 50% repeat');
			//ul.fadeOut(100);
			formChange();
			return false;
		});
		...
		texturePicker.click(function() {
			...
			jQuery('div.texturePicker ul:visible').hide().parent().css('position', 'static');
			if(showIt == true){
	...
	jQuery('div.theme-group .theme-group-header').addClass('corner-all').spinDown();

	// change event in form
	jQuery('#themeRoller form').bind('change', function() {
		formChange();
		return false;
	});
	
	// return key triggers form change
	jQuery.hotkeys.add('return', function () { formChange(); });

	//DL theme button
	jQuery('#downloadTheme').click(function(){
		var href = jQuery('link[href*=parseTheme.css.php]:last').attr('href');
		href = href.replace('','');
...
function themeGalleryBehaviors() {
	...
	jQuery('#themeGallery a')
		...
		.each(function(){
			var straightToDownload = (bookmarklet) ? '&ui-version=1.7&download=true' : '';
			jQuery(this).after(
			'<a href="/download/?themeParams=' + escape( '?'+ jQuery(this).attr('href').split('?')[1] ) + straightToDownload + '" class="download" title="Download this theme">Download</a>'+
		...
		.click(function(){
			...
			jQuery('#rollerTabs').tabs( "select", 0 );
			return false;
		});
...
jQuery(function() {
	...
	//app tabs
	jQuery('#rollerTabs').tabs({
		load: function(e, ui){
			
			rollYourOwnBehaviors();
			if(openGroups.length > 0){
				openGroups.join(','); jQuery('.theme-group-content:eq('+openGroups+')').prev().trigger('click');
			}
			if(focusedEl){
				jQuery('form input, form select, form .texturePicker').eq(focusedEl).click();
			}
			openGroups = [];
			focusedEl = null;
		...
		select: function(e,ui){
			if(jQuery(ui.panel).is('#rollYourOwn')  && hash.currHash() != hash.currentTabHash){ // Stop if we actually don't have a hash change
				hash.refreshToHash();
			}
	...
	jQuery('#help a[href="#rollYourOwn"]').click(function(){
		jQuery('#rollerTabs').tabs( "select", 0 );
		return false;
	});
	
	//links to theme gallery from help tab
	jQuery('#help a[href="#themeGallery"]').click(function(){
		jQuery('#rollerTabs').tabs( "select", 1 );
		return false;
	});
...
jQuery.hotkeys.remove('Ctrl+c', {target:'div#editor', type:'keypress'}); 
